...
PalladiumEvents.registerAnimations((event) => {
    event.registerForPower('ultimatrix_scan_mode/look', 'omni_extras:not_aliens/ultimatrix_scan_mode', 200, (builder) => {
        let activation = animationUtil.getAnimationTimerAbilityValue(
            builder.getPlayer(), 'omni_extras:not_aliens/ultimatrix_scan_mode', 'dummy', builder.getPartialTicks());
        if (activation > 0 && !builder.isFirstPerson()) {
            builder.get('left_arm')
                .setXRotDegrees(-124)
                .setYRotDegrees(22)
